...
### Normality check, raw data

This is only necessary if residuals of our selected model fail normality tests

```{r}
normal.max <- datTot %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
normal.max
### affinis and nigrensis fail normality; latipinna fine

normal.min <- datTot %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmin.C)$statistic,
          p.value = shapiro.test(CTmin.C)$p.value)
normal.min
### latipinna fail normality; affinis and nigrensis fine

normal.tot <- datTot %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(T.tot)$statistic,
          p.value = shapiro.test(T.tot)$p.value)
normal.tot
### all fine


hist.max <- ggplot(datTot, aes(x = CTmax.C)) +
  geom_histogram(bins = 15, fill = "grey", color = "black") +
  facet_wrap(~Species) +
  theme_minimal()
hist.max

hist.min <- ggplot(datTot, aes(x = CTmin.C)) +
  geom_histogram(bins = 15, fill = "grey", color = "black") +
  facet_wrap(~Species) +
  theme_minimal()
hist.min

hist.tot <- ggplot(datTot, aes(x = T.tot)) +
  geom_histogram(bins = 15, fill = "grey", color = "black") +
  facet_wrap(~Species) +
  theme_minimal()
hist.tot
...
qq.max <- ggplot(datTot, aes(sample = CTmax.C)) +
  geom_qq() + geom_qq_line() +
  facet_wrap(~Species) +
  theme_minimal()
qq.max

qq.min <- ggplot(datTot, aes(sample = CTmin.C)) +
  geom_qq() + geom_qq_line() +
  facet_wrap(~Species) +
  theme_minimal()
qq.min

qq.tot <- ggplot(datTot, aes(sample = T.tot)) +
  geom_qq() + geom_qq_line() +
  facet_wrap(~Species) +
  theme_minimal()
qq.tot
...
Will try a log transformation since it is most common and should deal with the slightly left skewed data I have

```{r}
trans.TempData <- rawTempData
trans.TempData [, c(8:10)] <- log10(rawTempData[8:10])

#### recheck normality
trans.normal.max <- trans.TempData %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
trans.normal.max
...
trans2.TempData <- rawTempData

trans2.TempData[, c(8:10)] <- sqrt(rawTempData[8:10])

#### recheck normality 

trans2.normal.max <- trans2.TempData %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
trans2.normal.max
...
trans3.TempData <- rawTempData

trans3.TempData[, c(8:10)] <- '^'(rawTempData[8:10],1/3)

#### recheck normality

trans3.normal.max <- trans3.TempData %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
trans3.normal.max

### wtf, still no difference?

trans4.TempData <- rawTempData

trans4.TempData[, c(8:10)] <- log(rawTempData[8:10])

#### recheck normality

trans4.normal.max <- trans4.TempData %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
trans4.normal.max
...
trans5.TempData <- rawTempData

trans5.TempData[, c(8:10)] <- log2(rawTempData[8:10])

#### recheck normality

trans5.normal.max <- trans5.TempData %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
trans5.normal.max
...
trans6.TempData <- rawTempData

trans6.TempData[, c(8:10)] <- '^'(rawTempData[8:10], 2)

#### recheck normality

trans6.normal.max <- trans6.TempData %>%
  group_by(Species) %>%
summarize(statistic =shapiro.test(CTmax.C)$statistic,
          p.value = shapiro.test(CTmax.C)$p.value)
trans6.normal.max

### marginally better, but still nowhere close to normal
...
Anova(model.1red)
Anova(model.1red, type = 2)
Anova(model.1red, type=3)

anova(model.1red, model.1b)
