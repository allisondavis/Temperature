...
```{r}
# OPTION 1: download 'rate_calc.csv' into working directory 
rates <- read.csv("rate_calc.csv", header = TRUE, sep = ",", stringsAsFactors = TRUE)
head(rates)
...
#calculate average rate per trial type
trial.rates <- rates %>%
  group_by(Trial) %>%
...
```{r}
#calculate average rate per trial per species
trial.rates2 <- rates %>%
...
**Results:** Despite normality, we will use non-parametric tests for our subsequent comparisons due to a small sample size (16 trials in CTmax and 14 trials in CTmin).
...
kruskal.test(Rate~Species, data=min.rates) #significant (p=0.034)
...
pw <- pairwise.wilcox.test(min.rates$Rate, min.rates$Species, p.adjust.method = "BH")
