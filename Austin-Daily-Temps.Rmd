...
We were interested in how the temperature rates used in our CTmax and CTmin trials (+/- 0.3°C/min) compared to those our specimens were exposed to in their original habitat. We calculated the daily temperature change rates (°C/min) for 2020-2024 in Austin, TX (USA). We then isolated the highest positive and negative rates for each month in 2020-2024 to illustrate how our rates compare to past extreme weather events. 

# **Data**

Temperature data were collected from Camp Maybry in Austin, TX (USA) (NOAA, National Centers for Environmental Information, https://www.ncei.noaa.gov/access/search/data-search/local-climatological-data-v2). Data reflect dry bulb temperatures recorded every hour for all months in 2020-2024. 
...
```{r}
#OPTION 1: download 'Austin_temp-data.csv' into working directory
temp <- read.csv("Austin_temp-data.csv", header = TRUE, sep = ",", stringsAsFactors = TRUE)
head(temp)
...
#not all time points have a recorded temperature; remove incomplete rows
temp <- temp %>%
  drop_na() %>%
  filter(as.character(HOUR) != "HOUR") %>% #random duplicate header
  filter(as.character(HourlyDryBulbTemperature) != "")
...
temp <- temp %>%
  mutate(
    time_obj = parse_date_time(as.character(HOUR), orders = "HMS"),
    minutes = hour(time_obj) * 60 + minute(time_obj) + second(time_obj) / 60)%>%
  select(-time_obj) #no longer needed after calculating minutes
...
slopes.all <- temp %>%
  ...
  summarise(
    slope_deg_per_minute = {
      ...
      y <- minutes
      ok <- !is.na(x) & !is.na(y)
      if(sum(ok) > 1) {         #ensures no rate is calculated if only one temp is available for that day
        coef(lm(x[ok] ~ y[ok]))[2]
      } else {
        NA_real_}},
    .groups = "drop")
...
#create wide format for easier by year viewing
...
```{r}
#isolate extreme rates per month
...
#create wide data frame for easier by year viewing
...
#final table for easiest viewing
extremes.wide %>%
  gt() %>%
  tab_spanner(
    label = "2020",
...
#Isolating the most extreme averages by year 
...
#Isolating the most extreme averages overall
