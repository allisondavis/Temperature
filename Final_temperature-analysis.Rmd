...
Remove the '#' before `install.packages` if it is the first time installing package. 
...
#get n for each sex in each species
sex.df <- datTot %>%
  count(Species, Sex) %>%
  tidyr::pivot_wider(
    names_from  = Sex,
...
Simple test comparing male and female lengths

```{r}
#normality check by sex and species
normal <- datTot %>%
...
#non-parametric test for size difference by species
size.test <- datTot %>%
  group_by(Species) %>%
...
nM <- c(16, 9, 15) #number of males for each species
nF <- c(16, 8, 15) #number of females for each species

# Sigma values are the residual SD from the above linear models
...
#find average differences in real data
