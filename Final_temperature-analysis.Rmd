...
PW.sp.tot <- emmeans(mod3f, "Species", vcov. = robVCV.sp)
summary(PW.sp.tot) #EMM
pairs(PW.sp.tot) #Pairwise results
      
  ## Plotting EMM

PW.df <- as.data.frame(PW.sp.tot)
...
PW.sex.tot <- emmeans(SWmod3, "sex.2")
summary(PW.sex.tot) #EMM
pairs(PW.sex.tot) #Pairwise results
      
  ## Plotting EMM

PW.df <- as.data.frame(PW.sex.tot)

em.tot <- ggplot(PW.df, aes(x=sex.2, y=emmean)) +
  geom_point(size=2) +
  geom_errorbar(aes(ymin=lower.CL, ymax=upper.CL), width=0.1) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) +
  ylab("Predicted mean temperature (°C) \n") +
...
This section will have table and figures used for publication.
...
pw.ttot <- pairs(PW.sp.tot) %>%
  as.data.frame() %>%
  mutate(
    Trial = "Thermal Breadth",
    est_se = sprintf("%.2f (%.2f)", estimate, SE)) %>%
  select(Trial, contrast, est_se, p.value)

pw.all <- dplyr::bind_rows(pw.ctmax, pw.ctmin, pw.ttot) %>%
  mutate(
    p.display = case_when(
...
pw.sex.all <- dplyr::bind_rows(pw.sex.max, pw.sex.min, pw.sex.tot) %>%
  mutate(
    p.display = case_when(
      p.value < 0.001 ~ "*p* < 0.001",
      p.value < 0.05  ~ paste0(sprintf("%.3f", p.value)),
...
ggplot(dat.plot, aes(x = Sex.plot, y = CTmax.C)) +
  geom_rect(
    data = subset(datTot, Species == "Sailfin"),
    inherit.aes = FALSE,
    xmin = -Inf, xmax = Inf,
...
ggplot(dat.plot, aes(x = Sex.plot, y = CTmin.C)) +
  geom_rect(
    data = subset(datTot, Species == "Sailfin"),
    inherit.aes = FALSE,
    xmin = -Inf, xmax = Inf,
...
ggplot(dat.plot, aes(x = Sex.plot, y = T.tot)) +
  geom_rect(
    data = subset(datTot, Species == "Sailfin"),
    inherit.aes = FALSE,
    xmin = -Inf, xmax = Inf,
...
tot.plot <- ggplot(dat.plot.sex, aes(x = Sex.plot, y = T.tot)) +
  geom_boxplot() +
  geom_point(color= "#8D8680", size=2, alpha=0.5) +
  scale_x_discrete(
  labels = function(x) {
...
em.max <- em.max + scale_x_discrete(labels = my.labels)
em.min <- em.min + scale_x_discrete(labels = my.labels)
em.tot <- em.tot + scale_x_discrete(labels = my.labels)

(em.max + em.min + em.tot + plot_layout(axis_titles = "collect"))
...
size.test #not significant for swordtail and sailfin, but significant for mosquitofish; this may be due to the low sample size for mosquiofish compared to the other two species


datTot %>%
  group_by(Species) %>%
  summarise(
    delta = effsize::cliff.delta(Size.mm ~ Sex, data = cur_data())$estimate,
    CI_low = effsize::cliff.delta(Size.mm ~ Sex, data = cur_data())$conf.int[1],
    CI_high = effsize::cliff.delta(Size.mm ~ Sex, data = cur_data())$conf.int[2]
  )
